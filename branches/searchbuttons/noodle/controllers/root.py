...
class RootController(BaseController):
    ...
    @expose()
    def search(self,query,submit):
        import logging
        log = logging.getLogger("search")
        log.info(submit)
        if submit == "noodle by file":
            #return search_by_file(self,query)
            redirect("/search_by_file?query=%s"%query)
        else:
            #return search_by_host(self,query)
            redirect("/search_by_host?query=%s"%query)
